...
class single_source_top_k():
    ...
    def generalized_bfs(self):
        ...
        for ngx in self.graph.iterNeighbors(self.root):
            tpl = (1,ngx,tuple([self.root,ngx]),{self.root,ngx})
        ...
        while len(self.PQ)>0:
            ...
            wgt, vtx, ptx, setptx = self.PQ.popleft()
    ...
    def standard(self,WEIG,VERT,PATH,PATHSET):
        ...
        for ngx in self.graph.iterNeighbors(VERT):
            ...
            tpl = (WEIG+1,ngx,PATH+(ngx,),PATHSET.union({ngx}))
            assert tpl not in self.PQ
            self.PQ.append(tpl)
            assert list(self.PQ)==sorted(self.PQ,key=lambda x:x[0])
            if self.non_sat[ngx]:
    ...
    def beyond(self,WEIG,VERT,PATH,PATHSET):
        ...
        for ngx in neighbors:
            ...
            if self.non_sat[ngx]:
                self.queue_dist_profile[ngx].append(len(PATH))
            assert list(self.queue_dist_profile[ngx])==sorted(self.queue_dist_profile[ngx])
...
if __name__ == "__main__":
    ...
    statsfile = str(os.path.basename(args.g))+"_"+str(K)+"_kbfs_"+date_time+'.csv'
