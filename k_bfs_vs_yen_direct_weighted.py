...
class yens_algorithm():
    ...
    def yen_pred(self,source,target,bound_on_length):
        ...
        pred={source:None}
        dist=[sys.maxsize for _ in self.G.iterNodes()]
        dist[source]=0
        fringe=[]
        heappush(fringe,(0,source))
        
        while len(fringe)>0:
            wgt,v=heappop(fringe)
            
            
            if(wgt>=bound_on_length):
                break
           
            if(v==target):
             
                assert wgt<bound_on_length
            ...
            for w in self.G.iterNeighbors(v):
                ...
                if dist[w]>wgt+self.G.weight(v, w):
                    pred[w]=v
               
                    dist[w]=wgt+self.G.weight(v, w)
                    heappush(fringe,(wgt+self.G.weight(v, w),w))
                   
        del dist            
        del fringe
        
        raise NoPathException(f"No path between {source} and {target}.")       
    def dijkstra(self,source,target, bound):
        wgt,pred=self.yen_pred(source, target, bound)
        path= deque()
        w=target
        while w is not None:
            path.appendleft(w)
            w=pred[w]
        assert sum([self.G.weight(path[i], path[i+1]) for i in range(0,len(path)-1)])==wgt
        assert wgt<bound
        return wgt,tuple(path)
    ...
    def run(self, source, target):
        ...
        try:
            ...
            wgt,p = self.dijkstra(source,target,sys.maxsize)
            assert p[0]==source
            assert p[-1]==target
            # assert w == len(p)-1
            
            assert p not in paths
            heappush(yen_PQ, (wgt,p))
            paths.add(p)
        ...
        while len(yen_PQ)>0:
           ...
           # assert sorted(results,key=len)==results
            ...
            for index in range(1, len(P_det)):
                ...
                try:
                    w_l=0
                    for a in range (0,len(l_root)-1):
                        w_l+=self.G.weight(l_root[a], l_root[a+1])
                    wgt,pgt = self.dijkstra(l_root[-1],target,sys.maxsize)
                    ...
                    heappush(yen_PQ,(w_l+wgt,new_path))
                    paths.add(new_path)
...
def read_nde(filename:str):
    with open(filename,'r', encoding='UTF-8') as f:
        ...
        return grafo_
def weight_find(path):
    w=0
    for a in range(0,len(path)-1):
       
        w+=G.weight(path[a],path[a+1])
    return w    
def test_equality(flag, array_A, array_B):
    ...
    if flag == "BY":
        ...
        for indice in range(len(array_yen)):
            if weight_find(array_yen[indice])!=weight_find(array_bfs[indice]):
                for xy in array_yen:
                ...
                print("index",indice)
                print(array_yen[indice],"Y",weight_find(array_yen[indice]))  
                print(array_bfs[indice],"B",weight_find(array_bfs[indice]))  
        ...
        indice = 0
        for indice in range(len(array_yen)):
            if weight_find(array_yen[indice])!=weight_find(array_yen_custom[indice]):
                print("Y",array_yen)
...
class single_source_top_k():
    ...
    def generalized_bfs(self):
        ...
        for ngx in self.graph.iterNeighbors(self.root):
            w=self.graph.weight(self.root, ngx)
            assert (w,ngx,tuple([self.root,ngx]),{self.root,ngx}) not in self.PQ
            #self.PQ.put((w,ngx,tuple([self.root,ngx]),{self.root,ngx},"reg",None,None))
            heappush(self.PQ, (w,ngx,tuple([self.root,ngx]),{self.root,ngx},"reg",None,None))
#            assert list(self.PQ)==sorted(self.PQ,key=lambda x:x[0])
            assert self.non_sat[ngx]
            self.queue_dist_profile[ngx].append((w,(self.root,)))
           # heappush(self.queue_dist_profile[ngx],(w,(self.root,)))
            assert sorted(self.queue_dist_profile[ngx])==list(self.queue_dist_profile[ngx])    

        while len(self.PQ)>0:
            
            wgt, vtx, ptx, setptx, flag, source, paths = heappop(self.PQ)
            ...
            if len(self.top_k[vtx]) < self.kappa:
                ...
                if(flag=="spec" and vtx in self.predecessors_set[source] and True):
                    ...
                    for ngx in neighbors:
                        w=self.graph.weight(vtx, ngx)
                        assert ngx not in setptx
                        assert ngx not in ptx

                        assert ngx in self.graph.iterNeighbors(vtx)
                        assert ptx[-1]==vtx
                        assert (wgt+w,ngx,ptx+(ngx,),setptx.union({ngx})) not in self.PQ
                       # self.PQ.append((wgt+w,ngx,ptx+(ngx,),setptx.union({ngx}),"spec",source,self.path_to_add[ngx]))
                        heappush(self.PQ, (wgt+w,ngx,ptx+(ngx,),setptx.union({ngx}),"spec",source,self.path_to_add[ngx]))
                        self.path_to_add[ngx]=[]
                       # assert list(self.PQ)==sorted(self.PQ,key=lambda x:x[0])
                        if self.non_sat[ngx]:
                            
                            # if(self.detour_done[ngx] ):
                                self.binary_search_alt(self.queue_dist_profile[ngx], 0, len(self.queue_dist_profile[ngx])-1,wgt+w, ptx)
                                # if(self.last_det_path[ngx]<wgt+w):
                            ...
                            #         heappush(self.queue_dist_profile[ngx], (wgt+w,ptx))
    ...
    def binary_search_alt(self, arr, low, high,w, x):
       
       
        index = bisect.bisect_left(arr,w,key=lambda x:x[0])
        
        while index<len(arr) and arr[index][0]==w:
            if arr[index][1]==x:
                assert (w,x) in arr
        ...
        assert (w,x) not in arr
       
        arr.insert(index,(w,x))
        if(len(arr)>self.kappa):
            arr.pop()
      
        assert len(arr)<= self.kappa 
        assert sorted(arr,key=lambda x:x[0])==list(arr)
      #  assert all (arr[i][0]==sorted(arr,key=lambda x:x[0])[i][0] for i in range(len(arr)))
    ...
    def standard(self,WEIG,VERT,PATH,PATHSET,flag,source,paths):
        ...
        self.top_k[VERT].append((WEIG,PATH))
       ...
       # assert PATH[-2] in self.graph.iterNeighbors(VERT)
        ...
        for ngx in self.graph.iterNeighbors(VERT):
            w=self.graph.weight(VERT, ngx)
            if ngx in PATHSET:
                continue
            assert  ngx != self.root
            assert (WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx})) not in self.PQ
            if ngx in neighbors:
              #  self.PQ.append((WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),flag,source,self.path_to_add[ngx]))
                heappush(self.PQ, (WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),flag,source,self.path_to_add[ngx]))
                self.path_to_add[ngx]=[]
            else:
              #  self.PQ.append((WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),"reg",None,None))
                heappush(self.PQ, (WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),"reg",None,None))
          #  assert list(self.PQ)==sorted(self.PQ,key=lambda x:x[0])
            if self.non_sat[ngx]:
                
                # if(self.detour_done[ngx] and not(flag=="reg" and WEIG+w==self.last_det_path[ngx])):
                    self.binary_search_alt(self.queue_dist_profile[ngx], 0, len(self.queue_dist_profile[ngx])-1,WEIG+w, PATH)
                
                    # if(self.last_det_path[ngx]<WEIG+w):
                    #     self.last_det_path[ngx]=0
                    #     self.detour_done[ngx]=False
                  
                # else:
                #     if(len(self.queue_dist_profile[ngx])<self.kappa):
                #        # self.queue_dist_profile[ngx].append((WEIG+w,PATH))
                #         print("BEFORE HEAPPUSH", list(self.queue_dist_profile[ngx]))
                #         heappush(self.queue_dist_profile[ngx], (WEIG+w,PATH))
                #         print("AFTER HEAPPUSH", list(self.queue_dist_profile[ngx]))
                #   #  assert all (len(self.queue_dist_profile[ngx][i])==len(sorted(self.queue_dist_profile[ngx],key=len)[i])for i in range(len(self.queue_dist_profile[ngx])))
                #     print("sort",sorted(self.queue_dist_profile[ngx],key=lambda x:x[0]))
                #     print("unsort",list(self.queue_dist_profile[ngx]))
                #     print(PATH)
                    assert sorted(self.queue_dist_profile[ngx],key=lambda x:x[0])==list(self.queue_dist_profile[ngx]) 
    ...
    def beyond(self,WEIG,VERT,PATH,PATHSET,flag,paths):
        ...
        if self.pigreco_set[VERT] is not None:
            ...
            while it_index < len(self.pigreco_set[VERT]):
                ...
                for DET in self.find_detours(VERT,vr,max_to_generate,WEIG,PATH,count_value):
                    if DET is None:
                        break
        ...
        for ngx in neighbors:
            ...
            assert PATH[-1]==VERT
            assert (WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx})) not in self.PQ
          #  self.PQ.put((WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),"spec",VERT,self.path_to_add[ngx])) 
            heappush(self.PQ, (WEIG+w,ngx,PATH+(ngx,),PATHSET.union({ngx}),"spec",VERT,self.path_to_add[ngx]))
            self.path_to_add[ngx]=[]
           # assert list(self.PQ)==sorted(self.PQ,key=lambda x:x[0])
            if self.non_sat[ngx]:
    ...
    def find_detours(self,source,target,at_most_to_be_found,dist_path,coming_path,count_value):
        ...
        if(len(self.queue_dist_profile[target])==self.kappa):
            bound=self.queue_dist_profile[target][self.kappa-1][0]
        else:
            bound=sys.maxsize
            
        if(bound==self.top_k[target][-1][0]):
        ...
        try:
            ...
            w,alt_p = self.dijkstra(source,target,bound-dist_path)
            
            assert alt_p[0]==source
            assert alt_p[-1]==target
            # assert alt_w == len(alt_p)-1  
           
            assert w+dist_path<bound
            assert alt_p not in paths
            heappush(yen_PQ,(w,alt_p))
            paths.add(alt_p)
        ...
        while len(yen_PQ)>0:
            ...
            w_det , P_det = heappop(yen_PQ)
            
            
            if len(self.queue_dist_profile[target])-self.count(target)>=at_most_to_be_found and w_det+dist_path>=bound:
                del yen_PQ
                self.distance_profile.clear()
                self.detours.clear()
                paths.clear()
            ...
            assert w_det+dist_path<bound
           
            if self.binary_search_alt(self.queue_dist_profile[target], 0, len(self.queue_dist_profile[target])-1,dist_path+w_det, coming_path[:-1]+P_det[:-1]):
            ...
            # if(w_det+dist_path>self.last_det_path[target]):
            #     self.last_det_path[target]=w_det+dist_path    
            yield P_det
           ...
           # assert sorted(self.detours,key=len)==self.detours
            ...
            for index in range(1, len(P_det)):
                ...
                try:
                    w_l=0
                    for a in range (0,len(l_root)-1):
                        w_l+=self.graph.weight(l_root[a], l_root[a+1])
                 
                    w_p,pgt = self.dijkstra(l_root[-1],target,bound-(dist_path+w_l)) 
                    
                    assert len(l_root[:-1] + pgt)-1==len(pgt)-1+len(l_root[:-1])               
                   # assert len(pgt)-1+len(l_root[:-1])+dist_path<bound
                    assert w_l+w_p+dist_path<bound
                    
                    new_path = l_root[:-1] + pgt
                   
                    assert w_p+w_l<bound-dist_path
                    assert new_path[0]==source and new_path[-1]==target
                    assert self.is_simple(new_path)   
                    ...
                    heappush(yen_PQ, (w_p+w_l,new_path))
                    paths.add(new_path)
    ...
    def pred(self,source,target,bound_on_length):
       ...
       # visited=set()
        #visited.add(source)
        pred={source:None}
        dist=[sys.maxsize for _ in self.graph.iterNodes()]
        dist[source]=0
        fringe=[]
        heappush(fringe,(0,source))
        ...
        while len(fringe)>0:
            ...
            for w in self.graph.iterNeighbors(v):
               # assert self.graph.edgeId(v,w) == self.graph.edgeId(w,v)
                ...
                #     if(wgt+self.graph.weight(v, w))<bound_on_length and (wgt+self.graph.weight(v, w))<actual_dist :
                         
                #         assert self.is_simple(pt+(w,))
                #         actual_dist=(wgt+self.graph.weight(v, w))
                #         path=pt+(w,)
                #         find=True
                       #return wgt+self.graph.weight(v, w),pt+(w,)
                if dist[w]>wgt+self.graph.weight(v, w):
                    pred[w]=v
                # if w not in visited:
                    dist[w]=wgt+self.graph.weight(v, w)
                    heappush(fringe,(wgt+self.graph.weight(v, w),w))
                   # visited.add(w)
       # del dist            
        del fringe
    ...
    def dijkstra(self,source,target, bound):
        wgt,pred=self.pred(source, target, bound)
        path= deque()
        w=target
        while w is not None:
            path.appendleft(w)
            w=pred[w]
        assert sum([self.graph.weight(path[i], path[i+1]) for i in range(0,len(path)-1)])==wgt
        assert wgt<bound
        return wgt,tuple(path)
...
if __name__ == "__main__":
    ...
    if not G.isDirected() or not G.isWeighted():
    ...
    import os
    statsfile = str(os.path.basename(args.g))+"_"+str(K)+"_kbfs_global_direct_weighted_"+date_time+'.csv'
    ...
    if __debug__ and G.numberOfNodes()<100:
        ...
        fig.tight_layout()
        plt.show()
    #randomly_selected_sqrt_roots={0}    
    ...
    for first_root in randomly_selected_sqrt_roots:
        ...
        for second in range(G.numberOfNodes()):
           ...
           # top_k_yen=[tuple(i) for i in islice(nx.shortest_simple_paths(aux_graph, first_root, second, weight='weight'), K)]
            
            local_cpu_yen += (time.perf_counter()-cpu_yen_single_pair)
            
            assert all(type(i)==tuple for i in top_k_yen)
            mio_top_k=[]
            for a in SSTK.top_k[second]:
                mio_top_k.append(a[1])
            test_equality("BY",mio_top_k,top_k_yen)
            if __debug__:
               test_equality("YY",top_k_yen,[tuple(i) for i in islice(nx.shortest_simple_paths(aux_graph, first_root, second, weight='weight'), K)])
