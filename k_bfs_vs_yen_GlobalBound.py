...
class single_source_top_k():
        ...
        while len(self.PQ)>0:
            ...
            if len(self.top_k[vtx]) < self.kappa:
                ...
                if(flag=="spec" and vtx in self.predecessors_set[source] and True):
                    ...
                    for i in paths:
                            if(len(i)==0) or self.non_sat[i[-1]]==False or  len(i)>self.bound[i[-1]]:
                               continue
    ...
    def count(self,vr):
        ...
        count=0
        for i in self.queue_dist_profile[vr]:
            if wg==len(i):
                count+=1
            else:
                break
        ...
        if self.pigreco_set[VERT] is not None:
            ...
            while len(localPQ)>0:
                ...
                if self.non_sat[vr]:
                    ...
                    max_to_generate = self.kappa-(len(self.top_k[vr])+count_value)
                   # max_to_generate=self.kappa-len(self.top_k[vr])
                   # assert max_to_generate>0  #da provare a rimuovere 
    ...
    def find_detours(self,source,target,at_most_to_be_found,dist_path,coming_path,count_value):
        ...
        if(self.bound[target]==len(self.top_k[target][-1])-1):
        ...
        while len(yen_PQ)>0:
            ...
            if len(self.queue_dist_profile[target])-self.count(target)>=at_most_to_be_found and len(P_det)-1+dist_path>=len(self.queue_dist_profile[target][(count_value+at_most_to_be_found)-1]):
                del yen_PQ
                self.distance_profile.clear()
                self.detours.clear()
                paths.clear()
            ...
            for index in range(1, len(P_det)):
                ...
                try:
                    ...
                    pgt = self.bidir_BFS(l_root[-1],target,self.bound[target]-(dist_path+len(l_root[:-1]))) 
...
if __name__ == "__main__":
    ...
    statsfile = str(os.path.basename(args.g))+"_"+str(K)+"_kbfs_global_"+date_time+'.csv'
    ...
    # randomly_selected_sqrt_roots={324,130,190,887}  #testa sul critical   4
    ...
    for first_root in randomly_selected_sqrt_roots:
        ...
        #print("bin occurrance ",SSTK.total," of wich useless ",SSTK.bi," capped ",SSTK.capped," det ",SSTK.det,"extra",SSTK.extra_visits)
